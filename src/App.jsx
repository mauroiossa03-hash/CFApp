...
const injectStyles = () => {
  ...
  el.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes glow     { 0%,100%{box-shadow:0 0 20px ${C.blue}44;} 50%{box-shadow:0 0 40px ${C.blue}88;} }
    @keyframes float    { 0%,100%{transform:translateY(0);} 50%{transform:translateY(-6px);} }
    @keyframes glowPulse{ 0%,100%{box-shadow:0 4px 20px ${C.blue}55;} 50%{box-shadow:0 6px 32px ${C.blue}99;} }
    ...
    .btn-glow:hover { animation: none; transform: translateY(-2px); box-shadow: 0 10px 32px ${C.blue}77; }
    .btn-ghost {
    ...
    .card-hover:hover { transform: translateY(-3px); border-color: ${C.blue}; box-shadow: 0 12px 28px rgba(26,86,219,0.14); }
    .tag {
...
function LogoMark({ size = 36 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ flexShrink:0, display:"block" }}>
      <defs>
        <linearGradient id="lgBarA" x1="0" y1="1" x2="0" y2="0">
          <stop offset="0%" stopColor="#1E4D8B"/><stop offset="100%" stopColor="#3B82C4"/>
        </linearGradient>
        <linearGradient id="lgBarB" x1="0" y1="1" x2="0" y2="0">
      ...
      <g>
        ...
        <rect x="28" y="45" width="5.2" height="27" rx="1" fill="url(#lgBarA)"/>
        <rect x="35" y="35" width="5.2" height="37" rx="1" fill="url(#lgBarB)"/>
        <rect x="42" y="24" width="5.2" height="48" rx="1" fill="#EAF4FE"/>
        <rect x="49" y="27" width="5.2" height="45" rx="1" fill="url(#lgBarB)"/>
        <rect x="56" y="34" width="5.2" height="38" rx="1" fill="url(#lgBarB)"/>
        <rect x="63" y="44" width="5.2" height="28" rx="1" fill="url(#lgBarA)"/>
...
function Logo({ size = 20, compact = false }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:9 }}>
      ...
      {!compact && (
        <span style={{
          ...
          letterSpacing:-.5, color:C.white, lineHeight:1.25, paddingBottom:2,
          display:"inline-block", whiteSpace:"nowrap"
        }}>
          Odds<span style={{ color:C.blue }}>Finance</span>
        </span>
      )}
    </div>
  );
...
function Landing({ setScreen, lang, setLang }) {
  ...
  const features = t ? [
    { icon:"🎯", title:"Quiz mirati per topic", desc:"Oltre 2000 domande in stile esame su tutti e 10 gli argomenti, con spiegazioni dettagliate per ogni risposta." },
    { icon:"🃏", title:"Flashcard intelligenti", desc:"Memorizza formule e concetti chiave con un sistema di ripetizione che si concentra sulle tue aree deboli." },
    { icon:"⏱️", title:"Simulatore d'esame reale", desc:"180 domande, 2 sessioni da 270 minuti. Identica all'esperienza del giorno dell'esame CFA." },
    { icon:"📊", title:"Analytics dei progressi", desc:"Monitora accuratezza, tempo e padronanza per ogni topic. Sai sempre dove migliorare." },
  ] : [
    { icon:"🎯", title:"Topic-focused quizzes", desc:"2000+ exam-style questions across all 10 areas, with detailed explanations for every answer." },
    { icon:"🃏", title:"Smart flashcards", desc:"Memorize key formulas and concepts with spaced repetition that targets your weak areas." },
    { icon:"⏱️", title:"Real exam simulator", desc:"180 questions, 2 × 270-minute sessions. Identical to CFA exam-day experience." },
    { icon:"📊", title:"Progress analytics", desc:"Track accuracy, timing and mastery per topic. Always know what to improve." },
  ];
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", position:"relative", overflow:"hidden", paddingBottom:40 }}>
      ...
      <div style={{ position:"absolute", inset:0, zIndex:0, pointerEvents:"none" }}>
        ...
        {[...Array(6)].map((_,i)=>(
          <div key={i} style={{ position:"absolute", left:0, right:0, top:`${i*20}%`, height:1, background:`${C.border}66` }}/>
        ))}
      </div>
      ...
      <main style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", padding:"32px 24px 0", position:"relative", zIndex:1, textAlign:"center" }}>
        ...
        <h1 style={{
          fontFamily:"'Cormorant Garamond', serif", fontSize:"clamp(38px,8vw,66px)",
          fontWeight:700, lineHeight:1.12, marginBottom:20, color:C.navy,
          animation:"fadeUp .5s ease .1s both", paddingBottom:4
        }}>
        ...
        <p style={{ fontSize:16, color:C.gray, maxWidth:440, lineHeight:1.7, marginBottom:32, animation:"fadeUp .5s ease .2s both" }}>
          {t ? "Smetti di studiare alla cieca. Quiz adattivi, simulatore d'esame e analisi dei progressi su tutti e 10 i topic — costruiti per farti superare l'esame, non solo per farti esercitare." :
               "Stop studying blind. Adaptive quizzes, full exam simulator and progress analytics across all 10 topics — built to get you through the exam, not just to practice."}
        </p>

        <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", animation:"fadeUp .5s ease .3s both" }}>
          <button onClick={()=>setScreen("register")} className="btn-primary btn-glow" style={{ padding:"15px 34px", fontSize:16 }}>
            {t?"Inizia Gratis →":"Start Free →"}
          </button>
          <button onClick={()=>setScreen("pricing")} className="btn-ghost" style={{ padding:"15px 28px", fontSize:16 }}>
            {t?"Vedi i piani":"See pricing"}
          </button>
        </div>

        <div style={{ fontSize:12.5, color:C.gray, marginTop:16, animation:"fadeUp .5s ease .35s both" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16, marginTop:48, maxWidth:440, width:"100%", animation:"fadeUp .5s ease .4s both" }}>
          {[
            ...
            <div key={i} className="card card-hover" style={{ padding:"18px 12px", textAlign:"center" }}>
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:30, fontWeight:700, color:C.blue }}>{s.n}</div>
        ...
        <div style={{ marginTop:40, display:"flex", flexDirection:"column", alignItems:"center", gap:8, animation:"fadeUp .5s ease .45s both" }}>
          ...
          <div style={{ fontSize:13, color:C.grayHi, fontWeight:600, maxWidth:380 }}>
            {t?"\"Ho passato il Level 1 al primo colpo. Il simulatore è identico all'esame vero.\"":"\"Passed Level 1 first try. The simulator is identical to the real exam.\""}
          </div>
        ...
        <div style={{ marginTop:72, width:"100%", maxWidth:520 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            {features.map((f,i)=>(
              <div key={i} className="card card-hover" style={{ padding:"20px", textAlign:"left", display:"flex", gap:16, alignItems:"flex-start" }}>
        ...
        <div className="card" style={{ marginTop:56, width:"100%", maxWidth:520, padding:"36px 28px",
          ...
          <p style={{ fontSize:14, color:"#DBEAFE", marginBottom:24, lineHeight:1.6, position:"relative", zIndex:1 }}>
          ...
          <button onClick={()=>setScreen("register")} className="btn-glow" style={{ position:"relative", zIndex:1,
            padding:"15px 40px", fontSize:16, fontWeight:700, borderRadius:12, border:"none", cursor:"pointer",
            background:"#fff", color:C.blue, fontFamily:"'Syne',sans-serif" }}>
            {t?"Crea il tuo account gratuito →":"Create your free account →"}
          </button>
          <div style={{ fontSize:12, color:"#BFDBFE", marginTop:14, position:"relative", zIndex:1 }}>
            {t?"Garanzia 7 giorni soddisfatti o rimborsati sui piani Premium":"7-day money-back guarantee on Premium plans"}
          </div>
        </div>
        ...
        <div style={{ marginTop:48, paddingTop:24, borderTop:`1px solid ${C.border}`, width:"100%", maxWidth:520,
          ...
          <div style={{ fontSize:12, color:C.gray }}>© 2026 OddsFinance</div>
        </div>
      </main>
    </div>
  );
      ...
      <div className="card" style={{ padding:"16px 20px", marginBottom:24, background:`linear-gradient(135deg, ${C.navy}, ${C.blue})`, borderColor:C.blue, animation:"fadeUp .4s ease .15s both" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:20, fontWeight:700, color:"#fff" }}>CFA Level I — Nov 2026</div>
          </div>
          <div style={{ textAlign:"center" }}>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:28, color:"#FCD34D", fontWeight:500 }}>
              {Math.max(0, Math.ceil((new Date("2026-11-15") - new Date()) / (1000*60*60*24)))}
            </div>
...
/* ── PRICING ── */
function PricingScreen({ lang, setIsPremium, user, setScreen }) {
  const t = lang==="it";
  ...
  const plans = [
    {
      ...
      features: t
        ? ["15 domande per topic","3 topic sbloccati","5 flashcard","Nessuna simulazione esame"]
        : ["15 questions per topic","3 unlocked topics","5 flashcards","No exam simulator"],
  ...
  return (
    <div style={{ padding:"24px 20px 90px", animation:"fadeIn .4s ease" }}>
      {!user && (
        <button onClick={()=>setScreen("landing")} className="btn-ghost" style={{ padding:"6px 14px", fontSize:12, marginBottom:16 }}>
          {t?"← Indietro":"← Back"}
        </button>
      )}
          ...
          {["monthly","annual"].map(b=>(
            <button key={b} onClick={()=>setBilling(b)}
              style={{
                ...
                color:billing===b?"#FFFFFF":C.gray, transition:"all .2s", fontFamily:"'Syne',sans-serif"
              }}>
          ...
          <div key={i} className="card" style={{
            ...
            {plan.badge && (
              <div style={{ position:"absolute", top:-12, left:"50%", transform:"translateX(-50%)" }}>
                <span className="tag" style={{ background:C.blue, color:"#FFFFFF", padding:"4px 14px" }}>{plan.badge}</span>
              </div>
            )}
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
              <div>
                <div style={{ fontSize:15, fontWeight:800, letterSpacing:.5, color:plan.primary?"#FFFFFF":C.navy }}>{plan.name}</div>
                {plan.price===0 && <div style={{ fontSize:12, color:C.gray, marginTop:2 }}>{t?"Per sempre gratis":"Free forever"}</div>}
              </div>
              <div style={{ textAlign:"right" }}>
                <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:700, color:plan.primary?"#FFFFFF":C.navy }}>
                  {plan.price===0?"€0":`€${plan.price}`}
                </span>
                <span style={{ fontSize:12, color:plan.primary?"#BFDBFE":C.gray }}>{plan.period}</span>
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
              {plan.features.map((f,j)=>(
                <div key={j} style={{ display:"flex", gap:8, alignItems:"center", fontSize:13 }}>
                  <span style={{ color:plan.primary?"#4ADE80":C.green, fontSize:14 }}>✓</span>
                  <span style={{ color:plan.primary?"#EAF2FF":C.grayHi }}>{f}</span>
                </div>
              ))}
              {plan.locked?.map((f,j)=>(
            ...
            <button onClick={()=>{
              ...
              style={{ width:"100%", padding:"13px", fontSize:14, cursor:"pointer",
...
function LoginScreen({ setScreen, setUser, lang }) {
  ...
  const handleLogin = async () => {
    ...
    setUser({ id: data.user.id, email: data.user.email, name: data.user.user_metadata?.full_name || data.user.email.split("@")[0], isPremium: false });
    setLoading(false);
  };
  ...
  return (
    <div style={{ padding:"24px 20px 90px", animation:"fadeIn .4s ease" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:2, marginBottom:20 }}>
        {[
          { icon:"🌐", label:t?"Lingua":"Language", val:lang==="it"?"Italiano":"English", action:()=>setLang(lang==="it"?"en":"it") },
          { icon:"📅", label:t?"Data esame":"Exam date", val:"Nov 2026", action:null },
